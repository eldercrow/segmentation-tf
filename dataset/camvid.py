...
class CAMVIDSegmentation(object):
    def __init__(self, basedir, name):
        ...
        fn_imageset = os.path.join(self._basedir, name.split('_')[1] + '.txt')
        ...
        self._imageset = [(_process(l[0]), _process(l[1])) for l in list_all]
    ...
    def _use_absolute_file_name(self, img):
        ...
        img['id'] = os.path.splitext(os.path.split(fn_img)[-1])[0] # just filename
    ...
    def load_many(basedir='', names=[], add_gt=True):
        ...
        # to simplify things
        if not basedir:
            basedir = cfg.DATA.CAMVID.BASEDIR
        if isinstance(names, str) and names in ('train', 'test'):
            names = getattr(cfg.DATA.CAMVID, names.upper())

        if not isinstance(names, (list, tuple)):
            names = [names]
        ret = []
        for n in names:
            db = CAMVIDSegmentation(basedir, n)
            ret.extend(db.load(add_gt))
        return ret


if __name__ == '__main__':
    basedir = os.path.expanduser('~/dataset/CamVid')
    names = ['camvid_train']
    annots = CAMVIDSegmentation.load_many(basedir, names)
